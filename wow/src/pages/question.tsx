...
const QuizPage: React.FC = () => {
    ...
    const handleNextQuiz = async () => {
        if(quiz && currentQuizIndex < quiz.length - 1){
            ...
            // 経験値加算APIを呼び出す
            if (session?.user?.id && correctDifficulties.length > 0) {
                try {
                    await fetch('/api/exp_status', {
                        method: 'POST',
    ...
    if (!quiz || quiz.length === 0) {
        return (
            <div style={{ textAlign: 'center', marginTop: '50px' }}>
                <h2>出題できるクイズがありません。</h2>
                <p>クイズを始めるには、まず単語を登録してください。</p>
